...
RSpec.feature "Comments::Questions", type: :feature do
  ...
  describe 'Create' do
    ...
    example '正常に作成できること' do
      within('.question_container') do
        ...
        expect{click_button 'コメントする'}.to change{ question.comments.count }.by(1)
      end
    ...
    example '空白のまま送信した場合、失敗すること' do
      within('.question_container') do
        expect{ click_button 'コメントする' }.to change{ question.comments.count }.by(0)
      end
  ...
  describe 'Destroy' do
    ...
    example 'コメントした本人ならば、コメントを削除できること' do
      expect{ find(".delete_comment_#{comment2.id}").click }.to change{ question.comments.count }.by(-1)
    end
